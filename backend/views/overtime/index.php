...
/* @var $searchModel backend\models\Overtime */
/* @var $dataProvider yii\data\ActiveDataProvider */

// 获取当前URL
$url = Yii::$app->request->url;
if($url == '/overtime/index'){
    $username = '个人加班管理';
}else{
    $user = new UserModel();
    $username = $user::find()->where(['id' => $_SESSION['__id']])->one()->username.'-的个人加班列表';
}
$this->title = Yii::t('common',$username);
$this->params['breadcrumbs'][] = $this->title;
?>
<div class="overtime-index">
    <?php  if(Helper::checkRoute('create')):?>
    <p class="clearfix">
        <?= Html::a('<i class="fa fa-plus"></i> '.Yii::t('common','　个人加班列表'), ['create'], ['class' => 'btn btn-success pull-right']) ?>
    </p>
    <?php  endif?>
    
    <!-- <?php //p(Helper::checkRole('update',['id'=>$_SESSION['__id']])); ?> -->
        ...
        'columns' => [
            ...
            [
                ...
                'value' => function($model){
                    return date('Y-m-d h:i:s',$model->fill_time);
                }
            ],
            'work_time',
            'work_address',
            ...
            'buttons' => [
                'update'=> function($url,$model,$key){
                    if(Helper::access('/overtime/create') && yii::$app->request->url == '/overtime/index'){
                        return Html::a(Yii::t('common','Update'),$url,['class'=>'btn btn-primary btn-sm']);
                    }
                },
                'view' => function($url,$model,$key){
                    if(Helper::access('/overtime/view')){
                        return Html::a(Yii::t('common','View'),$url,['class'=>'btn btn-warning btn-sm']);
                    }
                },
                'delete' => function($url,$model,$key){
                    if(Helper::access('/overtime/delete') && yii::$app->request->url == '/overtime/index'){
